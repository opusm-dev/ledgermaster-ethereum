...
const TableStore = artifacts.require('DataTableStore');
...
contract('TableStore', (accounts) => {
  ...
  beforeEach('', () => {
    return TableStore.new().then(ts => tableStore = ts);
  });

  it('test register / deregister table', async() => {
    const tables = Array(N_TABLE).fill().map(() => ({ name: nameGen(), column: nameGen() }));
    await Promise.all(tables.map(t => createTable(tableStore, t.name, t.column)))
