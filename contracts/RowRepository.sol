...
interface RowRepository {
  ...
  function findBy(string calldata _column, string calldata _start, int _st, string calldata _end, int _et, int _orderType) external view returns (table.Row[] memory);
  function countBy(string calldata _column, string calldata _start, int _st, string calldata _end, int _et) external view returns (uint);
}